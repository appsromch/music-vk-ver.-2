...
- (void)connectionDidFinishLoading:(CustomURLConnection *)connection
{
    ...
    if ([connection.tag isEqualToString:@"rData"]) {
        ...
        [audioArray removeObjectAtIndex:0];
...
- (void) saveFunc: (UIButton *) button {
    ...
    [progress setFrame:CGRectMake(self.view.frame.size.width - 140, 31, 133, 11)];
...
- (void) searchTableView {
    ...
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://api.vk.com/method/audio.search?access_token=%@&q=%@&auto_complete=1&sort=2&count=100", token, [self URLEncodedString:searchText]]]];
  ...
  /*  for (NSManagedObject *managedObject in self.fetchedResultsController.fetchedObjects) {
        ...
        if (titleResultsRange.length > 0 || titleRes.length > 0) {
            [audioArray addObject:managedObject];
        }
    } */
}
